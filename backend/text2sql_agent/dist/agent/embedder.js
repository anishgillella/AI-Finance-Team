...
import { SEMANTIC_MODELS } from "../data/semantic-models.js";
...
export class SemanticEmbedder {
    constructor() {
        this.model_name = "Xenova/all-MiniLM-L6-v2"; // 384-dimensional embeddings
        ...
        for (const [tableName, tableSchema] of Object.entries(schema)) {
            ...
            if (tableSchema.description) {
                try {
                    ...
                    const embedding = await this.embedText(`${tableName} - ${tableSchema.description}`);
                    results.push({
                        id: tableId,
            ...
            for (const [columnName, columnInfo] of Object.entries(tableSchema.columns)) {
                try {
                    ...
                    const text = `${tableName}.${columnName} - ${columnInfo.description || columnName} (${columnInfo.type})${columnInfo.unit ? ` [${columnInfo.unit}]` : ""}`;
