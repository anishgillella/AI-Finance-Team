...
export class QdrantVectorDB {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      await this.createCollection(
        this.SEMANTIC_MODELS_COLLECTION,
        "Semantic SQL patterns for finance queries"
      );

      // Create schema metadata collection
      await this.createCollection(
        this.SCHEMA_METADATA_COLLECTION,
        "Schema metadata (tables and columns) for the finance database"
      );
  ...
  private async createCollection(name: string, description: string): Promise<void> {
    try {
      ...
      const exists = Array.isArray(collections) && collections.some((c: any) => c.name === name);
   ...
   */
  async insertSemanticModels(points: QdrantPoint[]): Promise<number> {
    if (!this.initialized) {
    ...
    try {
      ...
      throw new Error(
        `Failed to insert semantic models: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
   ...
   */
  async insertSchemaMetadata(points: QdrantPoint[]): Promise<number> {
    if (!this.initialized) {
      await this.initialize();
    }
    ...
    try {
      ...
      throw new Error(
        `Failed to insert schema metadata: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
   ...
   */
  async searchSemanticModels(vector: number[], topK: number = 5): Promise<any[]> {
    if (!this.initialized) {
      await this.initialize();
    }
    ...
    try {
      ...
      throw new Error(
        `Failed to search semantic models: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
   ...
   */
  async searchSchemaMetadata(vector: number[], topK: number = 10): Promise<any[]> {
    if (!this.initialized) {
      await this.initialize();
    }
    ...
    try {
      ...
      throw new Error(
        `Failed to search schema metadata: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
   ...
   */
  private stringToId(str: string): string | number {
