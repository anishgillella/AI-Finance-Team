...
export async function runFinanceAgent(csvPath, query) {
    const state = {
        file_path: csvPath,
        chat_history: [
            {
...
${Object.entries(state.data.reduce((acc, t) => {
...
${Object.entries(state.data.filter(t => t.type === 'expense').reduce((acc, t) => {
                        acc[t.category] = (acc[t.category] || 0) + t.amount;
                        return acc;
                    }, {})).sort((a, b) => b[1] - a[1]).map(([cat, total]) => `• ${cat}: $${total}`).join('\n')}
