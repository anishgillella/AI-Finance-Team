...
import { FinancialRecord, KPI, AnomalyDetection } from '../types/index.js';
...
export const csvUploadTool = new DynamicTool({
  ...
  func: async (input: string) => {
    try {
      ...
      const parsed: FinancialRecord[] = records.map((record: any) => ({
        date: record.date || new Date().toISOString(),
        category: record.category || 'Uncategorized',
        amount: parseFloat(record.amount) || 0,
        description: record.description || '',
        type: (record.type || 'expense').toLowerCase() as 'income' | 'expense'
      }));
...
export const calculateKPIsTool = new DynamicTool({
  ...
  func: async (input: string) => {
    ...
    const records = parsed.data as FinancialRecord[];
...
export const anomalyDetectionTool = new DynamicTool({
  ...
  func: async (input: string) => {
    ...
    const threshold = 2;
    const anomalies: AnomalyDetection[] = records
